...
macro_rules! impl_static_cast {
    ($t:ty, $tbase:ty) => {
        impl $crate::widgets::StaticCastTo<$tbase> for $t {
...
macro_rules! impl_static_cast_propogate {
    ($t1:ty, $t2:ty, $t3:ty) => {
        impl $crate::widgets::StaticCastTo<$t3> for $t1 {
            fn static_cast(&self) -> &$t3 {
                <Self as $crate::widgets::StaticCastTo<$t2>>::static_cast(self).static_cast()
            }
