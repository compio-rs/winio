...
use compio_tls::{native_tls, TlsConnector, TlsStream};
use hyper::Uri;
use hyper_util::client::legacy::connect::{Connected, Connection};
use pin_project::pin_project;
use send_wrapper::SendWrapper;

use crate::{net::TcpStream, ui::AsRawWindow};
...
impl HttpStreamInner {
    pub async fn connect(uri: Uri, parent: impl AsRawWindow) -> io::Result<Self> {
        let scheme = uri.scheme_str().unwrap_or("http");
        let host = uri.host().expect("there should be host");
        let port = uri.port_u16();
        match scheme {
            "http" => {
                let stream = TcpStream::connect((host, port.unwrap_or(80)), &parent).await?;
                Ok(Self::Tcp(stream))
            }
            "https" => {
                let stream = TcpStream::connect((host, port.unwrap_or(443)), &parent).await?;
...
impl HttpStream {
    ...
    pub async fn connect(uri: Uri, parent: impl AsRawWindow) -> io::Result<Self> {
        Ok(Self::from_inner(
            HttpStreamInner::connect(uri, parent).await?,
        ))
    }
